    ...
    def train(self, iterations):        
        ...
        for episode in range(iterations):
            ...
            while s != None and time_step<200:
                ...
                self.W += self.beta * delta * phi_state(s)
                self.theta += self.alpha * I * delta * gradient
                I = self.gamma * I
                s = s_next
                time_step += 1 
...
def average_run(env, num_runs, episodes_per_run):
    ...
    for i in range (num_runs):
        ...
        agent = actor_critic_one_step(env)
...
def runningMean(x, N=10):
    ...
    return y/N
#%%
avg_returns = average_run(env, 5, 200)
#%%
reinforce_mean = runningMean(avg_returns, N=20)
#%%
# draw reward curves
plt.figure(1)
plt.plot(reinforce_mean, label='actor_critic_one_step')
plt.xlabel('Episodes')
