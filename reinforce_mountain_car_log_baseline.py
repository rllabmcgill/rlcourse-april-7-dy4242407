...
class REINFORCE_baseline:
    def __init__(self, environment, gamma=1, alpha=0.05, beta=0.1):
    ...
    def train(self, iterations):        
        ...
        for episode in range(iterations):
            ...
            for t in range(time_step):
               ...
               delta = G_t - self.cal_V(states[t])
               self.W += self.beta * delta * phi_state(states[t])
               ...
               self.theta += self.alpha * delta * gradient
...
def runningMean(x, N=10):
    ...
    return y/N
#%%
avg_returns = average_run(env, 3, 1000)
#%%
reinforce_mean = runningMean(avg_returns, N=100)
#%%
# draw reward curves
plt.figure(1)
plt.plot(reinforce_mean, label='REINFORCE_with_baseline')
plt.xlabel('Episodes')
